...
import com.video.youtuberplayer.model.YouTubeChannel;
...
public class SubscriptionsDb extends SQLiteOpenHelper {
  ...
  public static synchronized SubscriptionsDb getSubscriptionsDb() {
    if (subscriptionsDb == null) {
      subscriptionsDb = new SubscriptionsDb(VideoPlayerApplication.getContext());
    }
  ...
  private boolean hasVideo(YouTubeVideo video) {
    String query = String.format("SELECT COUNT(*) FROM %s WHERE %s = ?",
      SubscriptionsVideosTable.TABLE_NAME, SubscriptionsVideosTable.COL_YOUTUBE_VIDEO_ID);
